...
export function Profile() {
    ...
    const { userData, setUserPosts, note, setNote, setMessage, setStories, stories } = useUser();
    useEffect(() => {
    ...
    return <section className="mx-[12rem]">
        <div className="w-[62rem] pb-16 pt-12 border-b-[2px] border-[#262626]">
            <div className="flex gap-20 ml-16 items-center relative">
                ...
                <Link to={stories.length > 0 ? `/stories/${userData.data.user.userName}/${stories[0]._id}/` : ""} className={`p-2 ${stories.length > 0 ? "relative rounded-full multicolor-border" : ""}`}>
                ...
                <div className="flex flex-col gap-6">
                    <div className="flex gap-6 items-center">
                        <Link className="text-[20px] flex items-center gap-1">{userData?.data?.user.userName}
                            {userData.data.user.followers.length > 0 && <MdVerified className="fill-[#0095F6]" />}
                        </Link>
                        <div className="flex gap-3 items-center">
                            ...
                            <button className="bg-[#363636] px-5 py-1.5 rounded-[0.5rem] text-[14px] hover:bg-[rgb(38,38,38)] transition duration-150">View Archive</button>
                        </div>
                    </div>
                    <div className="flex gap-10 items-center">
                        <p className="flex gap-1.5"><span className="font-semibold">{formatNumber(userData.data.user.posts.length)}</span>posts</p>
                        <p className="flex gap-1.5"><span className="font-semibold">{formatNumber(userData.data.user.followers.length)}</span>followers</p>
                        <p className="flex gap-1.5"><span className="font-semibold">{formatNumber(userData.data.user.following.length)}</span>following</p>
                    </div>
                    <div>
            ...
            <div className="flex gap-10 ml-5 mt-16">
